...
User/custom mission scripts

--1--  CODE / SCRIPTS THAT NEEDS TO RUN ON THE (DEDICATED) SERVER
Here you'll place the code/scripts that need to be executed only by the server.
Such as spawning AI's/vehicles/objects. 

--2--  CODE / SCRIPTS THAT NEEDS TO RUN ON ONLY PLAYERS
...
// Reporting
if (ADF_debug || ADF_extRpt || time < 180) then {diag_log "ADF RPT: Init - executing: scripts\init.sqf"};
private _ds = diag_tickTime;
...
// Last line should be:
diag_log format ["ADF RPT: Init - FINISHED Scripts\init.sqf  [%1]",diag_tickTime - _ds];