...
// 5 min delayed start so that AO's can initialize before caching process starts
waitUntil {sleep 1; time > 300}; 

// Check if  the HC connected late. if that's the case than terminate caching.
if (ADF_HC_connected) exitWith {["zeb_cache - even though caching was enabled, an HC conntacted later resulting in the caching script to terminate.", true] call ADF_fnc_log;};
