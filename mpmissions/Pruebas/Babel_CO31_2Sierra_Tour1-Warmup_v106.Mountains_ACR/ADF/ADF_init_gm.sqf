...
// Reporting
if (ADF_debug || ADF_extRpt || time < 180) then {diag_log "ADF RPT: Init - executing: ADF_init_gm.sqf"};

// Init
if ((isNil "GM_1") && (isNil "GM_2")) exitWith {
	// No Zeus playable slots detected
	diag_log "ADF Debug: ZEUS - No GM units active. Terminating ADF_init_gm.sqf";
}; 
...
if !(isNil "GM_1") then {
	...
	ADF_GM_init = true;
	// Debug reporting
	if (ADF_debug) then {diag_log "ADF Debug: ZEUS: GM-1 active"};
};
...
if !(isNil "GM_2") then {
	...
	ADF_GM_init = true;
	// Debug reporting
	if (ADF_debug) then {diag_log "ADF Debug: ZEUS: GM-2 active"};
};
...
if ((isNil "GMmod_1") && (isNil "GMmod_2")) exitWith {
	// Debug reporting
	if (ADF_debug) then {diag_log "ADF Debug: ZEUS - ERROR! No Zeus Modules exist (GMmod_1/2). Terminating"};
};
...
if 	((!(isNil "GMmod_1") &&  !(isNil "GMmod_2")) && (!(isNil "GM_1") && !(isNil "GM_2"))) then {
	...
	// Debug reporting
	if (ADF_debug) then {diag_log "ADF Debug: ZEUS - Assigning GM_1 to GMmod_1, assigning GM_2 to GMmod_2"};
};
...
if 	(!(isNil "GMmod_1") && (!(isNil "GM_1") && (isNil "GM_2"))) then {
	...
	GM_1 assignCurator GMmod_1;
	// Debug reporting
	if (ADF_debug) then {diag_log "ADF Debug: ZEUS - Assigning GM_1 to GMmod_1. No GM_2 detected."};
};
...
if	(!(isNil "GMmod_2") && ((isNil "GM_1") && !(isNil "GM_2"))) then {
	...
	GM_2 assignCurator GMmod_2;
	// Debug reporting
	if (ADF_debug) then {diag_log "ADF Debug: ZEUS - Assigning GM_2 to GMmod_2. No GM_1 detected."};
};
...
if (((isNil "GMmod_1") && !(isNil "GMmod_2")) && (!(isNil "GM_1") && (isNil "GM_2"))) exitWith {
	if (ADF_debug || ADF_extRpt) then {diag_log "ADF Debug: ZEUS - ERROR! Cannot assign GM_1 to GMmod_2. Terminating"};
};
if ((!(isNil "GMmod_1") && (isNil "GMmod_2")) && ((isNil "GM_1") && !(isNil "GM_2"))) exitWith {
	if (ADF_debug || ADF_extRpt) then {diag_log "ADF Debug: ZEUS - ERROR! Cannot assign GM_2 to GMmod_1. Terminating"};
};

if (ADF_mod_Ares) exitWith { // Use ARES instead of ADF Zeus functions > V1.39 B7
	if (ADF_debug || ADF_extRpt) then {diag_log "ADF Debug: ZEUS - Ares addon detected. Using Ares instead of ADF Zeus functions"};
}; 
...
if (ADF_debug) then {diag_log "ADF Debug: ZEUS - ADV Zeus function Initialized"};
