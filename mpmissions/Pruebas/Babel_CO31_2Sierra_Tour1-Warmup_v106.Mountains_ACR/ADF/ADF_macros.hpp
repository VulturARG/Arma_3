...
#define ADF__SUPP(VAR,SCR) if !(isNil (str VAR)) then {[VAR] execVM format ["scripts\supplies\%1",SCR]}
...
#define ADF__ERPT(S) if (ADF_debug || time < 180) then {diag_log format ["ADF RPT: Init - executing: %1",S]}
