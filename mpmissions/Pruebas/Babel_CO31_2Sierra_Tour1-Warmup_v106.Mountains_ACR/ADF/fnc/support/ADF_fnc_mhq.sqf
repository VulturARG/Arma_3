...
Script: MHQ functions
Author: Whiztler
Script version: 1.33

File: ADF_fnc_MHQ.sqf
**********************************************************************************
DO NOT edit this file. To set-up and configure your mission, edit the files in
the  '\mission\'  folder.

Note these functions are loaded via the MHQ module.
*********************************************************************************/

// Reporting
if (ADF_debug || ADF_extRpt) then {diag_log "ADF RPT: fnc - executing: ADF_fnc_MHQ"};
...
ADF_fnc_MHQ_marker = {	
	...
	// MHQ is destroyed, change the Flag Marker
	"mMHQ" setMarkerColor "ColorGrey"; 
};


// MHQ vehicle respawn time as configured in mission\ADF_mission_settings.sqf
ADF_fnc_MHQ_respawn = {
	...
	if (isServer) then {
		// Create a new MHQ
		MHQ = createVehicle [ADF_MHQ_vehicle, [0,0,0], [], 0, "none"]; publicVariable "MHQ";
		MHQ setPos ADF_MHQ_orgPos;
		...
		MHQ addEventHandler ["killed", {[] spawn ADF_fnc_MHQ_respawnInit}];
		 // Load the supplies
		[MHQ] execVM "ADF\scripts\supplies\ADF_vCargo_B_MHQ.sqf";
	...
	if (hasInterface) then {
		...
		// Display a hint that the MHQ has re-spawned
...
ADF_fnc_MHQ_FinalSpawn = {
	...
	private _m = createMarker ["mFinalSpawn", getMarkerPos "respawn_west"];
	_m setMarkerShape "ICON"; 
	...
	if (ADF_clanFlag != "") then {
		ADF_respawnFlag = createVehicle ["Flag_White_F", [0,0,0], [], 0, "NONE"];
		ADF_respawnFlag setPos (getMarkerPos "respawn_west");
		ADF_respawnFlag setFlagTexture ADF_clanFlag;		
	} else {
		ADF_respawnFlag = createVehicle ["Flag_NATO_F", [0,0,0], [], 0, "NONE"];
		ADF_respawnFlag setPos (getMarkerPos "respawn_west");
	};
...
ADF_fnc_fobRemove = {
	// Init
	params ["s"];
	private _q = ["Flag_NATO_F", "Land_Pallet_MilBoxes_F", "CamoNet_BLUFOR_F", "B_supplyCrate_F", "Land_PaperBox_open_full_F", "Land_Cargo20_military_green_F", "Land_HBarrierBig_F", "Land_HBarrier_3_F"];
	private _o = nearestObjects [getPos MHQ, _q, 14];
	private _t = 0;	
