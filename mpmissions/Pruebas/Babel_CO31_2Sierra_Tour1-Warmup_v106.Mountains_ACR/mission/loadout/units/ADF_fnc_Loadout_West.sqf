...
ADF_fnc_loudoutInf = {
	...
	// headgear
	_ADF_inf_headgear = selectRandom ["H_HelmetB","H_HelmetB_plain_mcamo","H_HelmetSpecB_paint1","H_HelmetSpecB_paint2","H_Helmet_Kerry"];
	...
	if ((_r == "sql") || (_r == "ftl") || (_r == "xo") || (_r == "wtl") || (_r == "pc")) exitWith {
		...
		// XO baret
		[_ADF_unit] call ADF_loadOut_gearLoaded;
...
ADF_fnc_loadoutSor = {
	...
	// Face paint
	_ADF_SOR_face = selectRandom ["GreekHead_A3_10_sa","GreekHead_A3_10_l","GreekHead_A3_10_a","WhiteHead_22_a","WhiteHead_22_l","WhiteHead_22_sa"];
	...
	if (_r != "uav") then {	
		if ((_r == "ssc") || (_r == "rtl") || (_r == "rto")) then {	
			...
			if (!ADF_mod_TFAR && !ADF_mod_ACRE) then {_ADF_unit addBackpack "B_AssaultPack_blk"};
			if (ADF_microDAGR_all == 2) then {_ADF_unit addItemToUniform ADF_microDAGR};
		} else {
			if (_r != "rm") then {_ADF_unit addBackpack selectRandom ["B_TacticalPack_blk", "B_AssaultPack_blk", "B_FieldPack_blk"];} else {_ADF_unit addBackpack "B_TacticalPack_blk";};
		};
		// Store in Backpack
		_ADF_unit addItemToBackpack "MiniGrenade";
		_ADF_unit addItemToBackpack "MiniGrenade";
	};	
	_ADF_unit addVest "V_PlateCarrier2_blk";
	...
	// headgear
	_ADF_sor_headgear = selectRandom ["H_Bandanna_khk_hs","H_Watchcap_blk","H_Watchcap_cbr","H_Watchcap_khk","H_Bandanna_gry","H_Cap_usblack"];
	_ADF_unit addHeadgear _ADF_sor_headgear;
	// Upgrade NV
	if (_ADF_NVGoggles) then {
		_ADF_unit removeWeapon "NVGoggles";
		if (	((date select 3) > 5) && ((date select 3) < 19)) then {
			_ADF_unit addItemToBackpack "NVGoggles_OPFOR";
		} else {
			_ADF_unit addWeapon "NVGoggles_OPFOR";
		};		
	};
	// microDAGR
	if (ADF_microDAGR_all == 1) then {_ADF_unit addItemToUniform ADF_microDAGR};	
	...
	if (_r != "rmm") then {
		...
		if (ADF_dlc_MarksMan) then { // Check if Marksman DLC premium is active
			...
			_ADF_unit addPrimaryWeaponItem "bipod_01_F_blk";
...
ADF_fnc_loadoutSod = {
	...
	if (_r != "uav") then {	
		...
		_ADF_sor_headgear = selectRandom ["H_Watchcap_camo","H_Watchcap_blk","H_Booniehat_mcamo","H_Watchcap_khk","H_Bandanna_gry","H_Shemag_olive_hs","H_Bandanna_khk_hs"];
...
ADF_fnc_loadoutSop = {
	...
	// Face paint
	_ADF_SOP_face = selectRandom ["WhiteHead_22_a","WhiteHead_22_l","WhiteHead_22_sa"];
