...
{
 for "_j" from 1 to 8 do //Número de columnas
 {
  //_unit = createGroup west createUnit [selectRandom _classes, _pos vectorAdd [_i*2 + (0.2 - random 0.4), _j*4, 0], [], 0, "CAN_COLLIDE"];
	if (_i == 1) then {
			if (_j == 7 ) then {
				_unit = createGroup west createUnit ["arga_multicam_fusilero", _pos vectorAdd [_i*1.5 + (0.2 - random 0.4), _j*2.5, 0], [], 0, "CAN_COLLIDE"];
