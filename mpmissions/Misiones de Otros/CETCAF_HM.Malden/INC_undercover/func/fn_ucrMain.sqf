...
switch (_operation) do {
		...
		switch (!isPlayer _unit || {!(isClass(configFile >> "CfgPatches" >> "ace_interact_menu"))}) do {
			...
			case true: {
				...
				[_unit, [
					...
					"<t color='#334FFF'>Armas ocultas</t>", {
						...
						[_unit] spawn {
							...
							if !((currentWeapon _unit == "") || {currentWeapon _unit == "Throw"} || {currentWeapon _unit == binocular _unit}) then {
								switch (isPlayer _unit) do {
									case true: {
										hint "Algumas armas foram escondidas";
									};
									case false: {
										_unit groupChat "Não foi possível encaixar tudo.";
									};
								};
							};
						};
				...
				[_unit, [
					...
					"<t color='#FF33BB'>Tirar armas escondidas</t>", {
						...
						[_unit] spawn {
							...
							switch (currentWeapon _unit == primaryWeapon _unit) do {
								case true: {
									_unit groupChat "Tirei meu rifle.";
								};
								case false: {
									_unit groupChat "Tirei minha pistola.";
								};
							};
						};
			...
			case false: {
				...
				if !(_unit getVariable ["INC_addedAceActions",false]) then {
					...
					INC_concealWeaponsACE = ['Armas ocultas','Armas ocultas','',{
					...
					INC_unConcealWeaponsACE = ['Tirar armas','Tirar armas escondidas','',{
	...
	case "SwitchUniformAction": {
		...
		INC_switchUniformAction = _unit addAction [
			"<t color='#33FF42'>Encontre um novo disfarce proximo</t>", {
				params ["_unit"];
				...
				if (_success) then {
					if (!isPlayer _unit) then {
						private _comment = selectRandom ["Achei um.","Peguei algo","Isso vai servir","Encontrei algumas roupas."];
						_unit groupChat _comment;
					} else {hint "Uniforme alterado."};
				} else {
					if (!isPlayer _unit) then {
						private _comment = selectRandom ["Qual?","Não tenho certeza de onde você quer que eu olhe.","Você pode apontar um pouco melhor?"];
						_unit groupChat _comment;
					} else {hint "Nenhum uniforme seguro encontrado nas proximidades."};
				};
	...
	case "SwapGearAction": {
		...
		INC_stealGear = _unit addAction [
			"<t color='#33FF42'>Trocar Equipamento</t>", {
				params ["_unit"];
	...
	case "CheckDisguiseAction": {
		...
		INC_checkDisguise = _unit addAction [
			"<t color='#47AF24'>Verifique o Disfarce</t>", {
				params ["_unit"];
