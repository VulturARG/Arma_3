...
removeAllActions btc_gear_object;
//Re-deploy
...
if (btc_p_arsenal_Type < 3) then {
    btc_gear_object addAction [localize "STR_BTC_HAM_ACTION_ARSENAL_OPEN_BIS", "['Open', [!(btc_p_arsenal_Restrict isEqualTo 1), _this select 0]] call bis_fnc_arsenal;",[],1,false,true,"","_this distance _target < 5"];
};
//ACE
if (btc_p_arsenal_Type > 0) then {
    ...
    if (btc_p_arsenal_Type in [2, 4]) then {
        btc_gear_object addAction [localize "STR_BTC_HAM_ACTION_ARSENAL_OPEN_ACE", "[btc_gear_object, player] call ace_arsenal_fnc_openBox;",[],1,false,true,"","_this distance _target < 5"];
    };
};
